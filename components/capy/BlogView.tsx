...
export const BlogView: React.FC = () => {
  ...
  const addPlaybook = (slug: string) => {
    executeContextCommand('templates.addPlaybook', { slug });
  };
  ...
  return (
    <div className="p-4 flex flex-col h-full bg-ide-sidebar text-ide-text overflow-y-auto">
      ...
      <div className="space-y-4">
        {posts.map((post) => (
          <div key={post.slug} className="flex flex-col gap-2 pb-4 border-b border-ide-activity last:border-0">
            <h3 className="font-bold text-sm hover:text-ide-accent cursor-pointer">{post.title}</h3>
            <div className="flex justify-between items-center text-xs text-gray-500">
              <span>{post.date} • 5 min read</span>
              <button
                onClick={() => addPlaybook(post.slug)}
